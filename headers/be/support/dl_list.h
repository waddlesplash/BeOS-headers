...
#define REMOVE_HCDL_LIST( item, head, prefix ) \
	do { \
		if( item->prefix##next ) \
			item->prefix##next->prefix##prev = item->prefix##prev; \
		else \
			(head)->prefix##prev = item->prefix##prev; \
\
		if( item != (head) ) \
			item->prefix##prev->prefix##next = item->prefix##next; \
		else \
...
#define ADD_HCDL_LIST_TAIL( item, type, head, prefix ) \
	do { \
		type *old_head = (head); \
 \
...
#define ADD_HCDL_LIST_HEAD( item, type, head, prefix ) \
	do { \
		type *old_head = (head); \
 \
 		(head) = item; \
