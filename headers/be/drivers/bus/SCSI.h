/* 
	...
	For controller driver (SIM) writers: 
...
// path; used by legacy wrappers (uint8)
#define SCSI_DEVICE_PATH_ID_ITEM "scsi/path_id"
// node type
#define SCSI_DEVICE_TYPE "scsi/device"
...
// SCSI device interface for peripheral drivers
...
#define SCSI_DEVICE_INTERFACE "scsi/device/v1"
...
// node type
#define SCSI_BUS_TYPE "scsi/bus"

/*// SCSI bus interface for consumers.
// You never really want to talk to the bus directly but to a device.
...
#define SCSI_BUS_INTERFACE "scsi/bus/v1"*/


// cookie for dpc
typedef struct scsi_dpc_info *scsi_dpc_cookie;

// SCSI bus manager interface for SIM.
typedef struct scsi_bus_for_sim_interface {
...
#define SCSI_BUS_FOR_SIM_INTERFACE "scsi/bus/for_sim/v1"

// name of SCSI bus manager.
// must be specified as fixed consumer of SCSI HBA nodes
#define SCSI_BUS_MANAGER_MODULE_NAME "scsi/scsi_bus_manager/bus/v1"



// SCSI Host Bus Adapter driver (SIM) node

// attributes:

// node type
#define SCSI_SIM_TYPE "scsi/controller"

typedef struct scsi_sim_cookie *scsi_sim_cookie;

// SIM interface used by bus manager.
// SIMs must provide this interface for each bus
typedef struct scsi_sim_interface {
	...
	// called when first peripheral driver connects to device
	// (used by IDE layer to setup removable medias)
	void (*connect_device)(
		scsi_sim_cookie 	cookie, 
		uchar				target_id );	// target id
		
	// called when last peripheral driver disconnects from device
...
#define SCSI_SIM_INTERFACE "scsi/sim/v1"
