/* 
	...
	Part of Block device manager
	
	
	Block devices can be easily writting by providing the interface
	...
	protocoll and transparently handles transmission of partial blocks
...
// Interface to be provided by block device driver.
typedef struct blkdev_interface {
	...
	status_t (*read)( blkdev_handle_cookie handle, const phys_vecs *vecs, 
		off_t pos, size_t num_blocks, uint32 block_size, size_t *bytes_transferred );
	status_t (*write)( blkdev_handle_cookie handle, const phys_vecs *vecs, 
		off_t pos, size_t num_blocks, uint32 block_size, size_t *bytes_transferred );
...
#define BLKDEV_INTERFACE "blkdev"



// Block device manager node
...
typedef struct blkman_for_driver_interface {
	...
	void (*set_media_params)( blkman_device device, 
		uint32 block_size, uint32 ld_block_size, uint64 capacity );
} blkman_for_driver_interface;

#define BLKMAN_FOR_DRIVER_INTERFACE "blkman/driver/v1"

#define BLKMAN_MODULE_NAME "pnp_core/blkman/v1"
