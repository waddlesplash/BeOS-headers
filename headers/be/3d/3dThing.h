...
void B3dThing::LinkTo(B3dVector *axis, float speed, long mode, B3dVector *origin) {
	LinkTo(new B3dLink0(axis, speed, mode, origin));
}

void B3dThing::LinkTo(float speed_alpha, float speed_theta,
					  float speed_phi, B3dVector *origin) {
	LinkTo(new B3dLink1(speed_alpha, speed_theta, speed_phi, origin));
}

void B3dThing::LinkTo(B3dThing *from, B3dVector *to, B3dMatrix *to_mat, float speed) {
	LinkTo(new B3dLink2(from, to, to_mat, speed));
}
