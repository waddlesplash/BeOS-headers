...
/*
	ioctl() codes specific to ATA devices.
	Those existed before but were kept private (values are not the smartest).
	Used in ideinfo and idestatus
*/

enum {
	B_ATA_GET_DEVICE_INFO = B_DEVICE_OP_CODES_END + 1,	// ata_device_infoblock *
	B_ATA_GET_DEVICE_STATUS					// ata_device_status *
};

/*
	Definition of response to ATA_CMD_IDENTIFY_DEVICE or 
	ATA_CMD_IDENTIFY_PACKET_DEVICE
	Additionally, what is filled in by B_ATA_GET_DEVICE_INFO
...
typedef struct tagdevice_infoblock {
	...
	LBITFIELD16 (				// a4 (82) supported_command_set
		...
		PM_supported				: 1,
		_82_fixed					: 1,	// must be 0 for ATA, 1 for ATAPI
...
enum {
	B_ATA_DMA_STATUS_UNSUPPORTED,
	B_ATA_DMA_STATUS_ENABLED,
	B_ATA_DMA_STATUS_DISABLED_USER,
	B_ATA_DMA_STATUS_DISABLED_SAFE_MODE,
	B_ATA_DMA_STATUS_UNSUPPORTED_CONTROLLER,
	B_ATA_DMA_STATUS_DISABLED_DRIVER_CONFIG,
	B_ATA_DMA_STATUS_DISABLED_DRIVER_FAILURE
};

typedef struct tagata_device_status {
        uint8 version;	// 1
        uint8 dma_status;
        uint8 pio_mode;
        uint8 dma_mode;
} _PACKED ata_device_status;
